$(function () {
  ...
  function Create() {
    ...
    var person = JSON.stringify({
      ...
      Contrasenia1: $("#txtContrasenia1").val(),
      Phone: $("#txtPhone").val(),
      Email: $("#txtEmail").val(),
      Estado: "e",
      Tipo: $("#txtTipo").val()
  ...
  function List() {
    ...
    for (var i in tblUsuarios) {
        var per = JSON.parse(tblUsuarios[i]);
        $("#tblList tbody").append("<tr>" +                    
                "<td>" + per.ID + "</td>" +
                "<td>" + per.Name + "</td>" +
                "<td>" + per.Apellido + "</td>" +
                "<td>" + per.Contrasenia1 + "</td>" +
                "<td>" + per.Phone + "</td>" +
                "<td>" + per.Email + "</td>" +                    
                "<td>" + per.Estado + "</td>" +                    
                "<td>" + per.Tipo + "</td>" +                    
  ...
  $(".btnEdit").bind("click", function () {
    ...
    var per = JSON.parse(tblUsuarios[selected_index]); 
    $("#txtID").val(per.ID);
    $("#txtName").val(per.Name);
    $("#txtApellido").val(per.Apellido);
    $("#txtContrasenia1").val(per.Contrasenia1);
    $("#txtPhone").val(per.Phone);
    $("#txtEmail").val(per.Email);
    $("#txtEstado").val(per.Estado);
